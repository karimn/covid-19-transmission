...
#hierarch_args="--no-partial-pooling=trend --center-mobility-model --center-log-r0"
hierarch_args="--no-partial-pooling=trend"
mob_model_type="--mobility-model-type=${4}"

Rscript run_mob.R ${run_type} ${SLURM_ARRAY_TASK_ID} -i ${iter} --hyperparam=separate_hyperparam.yaml --show-script-options --include-param-trend $output_args $hierarch_args $mob_model_type
