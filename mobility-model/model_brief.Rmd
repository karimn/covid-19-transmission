...
In this section we will demonstrate some of the main results of the analysis described here, using data from `r country_name`. We were actually able to run this analysis on data from 19 countries.

In Figure \ref{fig:R0-plot}, we summarize the posterior distribution of $R_0$ across all sub-regions. 

```{r R0-plot, fig.width=10, fig.cap="$R_0$ across all sub-regions."}
results %>% 
  plot_subnat_ci(par = "R0") + 
  labs(title = TeX("R_{0,m}")) +
  theme(strip.text = element_blank()) +
  labs(caption = "Points represent the posterior median, diamonds the posterior mean, and line ranges are the 90% credible intervals.") +
  NULL
```

In Figure \ref{fig:Rt-plot}, we summarize the full time-series of $R_t$'s evolution across all sub-regions.

```{r Rt-plot, fig.height=8, fig.width=10, fig.cap="Adjusted $R_{t}$ across all regions."}
results %>% 
...
```{r post-deaths, fig.height=8, fig.width=10, fig.cap="Posterior predicted number of new deaths."}
results %>% 
  plot_post_deaths() +
