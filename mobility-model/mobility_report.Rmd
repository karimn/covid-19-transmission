...
params:
  ...
  results_file: se_64507172_109_mob_results.rds
  fit_file: se_64507172_109_mob.RData
  country_code: SE 
...
results %>% 
  ...
  unnest(country_data) %>% 
  plot_population(at_level = if (params$singletons) "country_name" else "sub_region")
```

```{r obs-cases, fig.width=15, fig.height=8}
results %>%
...
```{r obs-deaths, fig.width=15, fig.height=8}
results %>%
  select(country_code, country_data) %>% 
  unnest(country_data) %>% 
...
```{r mob-plot, fig.height=8, fig.width=12}
results %>% 
  select(country_code, country_data) %>% 
  unnest(country_data) %>% 
