...
subnat_data %<>%
  ...
  mutate(
    ...
    daily_data = pmap(lst(daily_data, first_infection_seeding_day, last_effective_observed_day, is_valid = has_epidemic), clean_date_ranges) %>%
      ...
      map(mutate, g_residential = - g_residential,
                  average_mob = (g_grocery_and_pharmacy + g_parks + g_retail_and_recreation + g_workplaces) / 4,
                  average_all_mob = (g_residential + g_transit_stations + g_grocery_and_pharmacy + g_parks + g_retail_and_recreation + g_workplaces) / 6) %>%
      map(arrange, date) %>%
